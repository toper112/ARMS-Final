            ...
            <div
                ...
                <div class="flex mb-4">
                    <a href="{{ route('admin.roles.index') }}"
                        class="px-4 py-2 bg-green-700 hover:bg-green-500 text-white rounded-md transition duration-150 ease-in-out">Role
                        Index</a>
                </div>
                <div class="flex flex-col bg-slate-100 p-4 rounded-md">
                    <div class="space-y-8 divide-y divide-gray-200 w-1/2 mt-10">
                        <form method="POST" action="{{ route('admin.roles.update', $role->id) }}">
                            ...
                            <div class="sm:col-span-6 pt-5">
                                <button type="submit"
                                    class="px-4 py-2 bg-green-500 hover:bg-green-700 text-white rounded-md transition duration-150 ease-in-out">Update</button>
                            </div>
                        </form>
                    </div>
                </div>
                <div class="mt-6 p-4 bg-slate-100 rounded-md">
                    <h2 class="text-2xl font-semibold dark:text-gray-300">Role Permissions</h2>
                        ...
                        @if ($role->permissions)
                            @foreach ($role->permissions as $role_permission)
                                <form method="POST"
                                    ...
                                    @method('DELETE')
                                    <button type="submit"
                                        class="px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-md transition duration-150 ease-in-out">{{ $role_permission->name }}</button>
                                </form>
                            @endforeach
                        @endif
                    </div>
                        ...
                        <form method="POST" action="{{ route('admin.roles.permissions', $role->id) }}">
                            ...
                            <div class="sm:col-span-6">
                                <label for="permission"
                                    class="block text-sm font-medium text-gray-700 dark:text-gray-300">Permission</label>
                            ...
                            <div class="sm:col-span-6 pt-5">
                                <button type="submit"
                                    class="px-4 py-2 bg-green-500 hover:bg-green-700 text-white rounded-md transition duration-150 ease-in-out">Assign</button>
                            </div>
                        </form>
                    </div>
                </div>
